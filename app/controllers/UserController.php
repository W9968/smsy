...
class UserController {
    ...
    public function store(User $req) {
        try {
            ...
            VALUES (NULL, ?, ?, ?, '1', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, current_timestamp())")->execute([$req->getCin(), $req->getPassword(), $req->getRole(), $req->getPicture(), $req->getFistName(), $req->getLastName(), $req->getEmail(), $req->getState(), $req->getState(), $req->getZip(), $req->getGender(), $req->getAddress1(), $req->getAddress2(), $req->getBirth(), $req->getPhone()
            ]);
        } catch (Exception $e) {
            echo $e->getMessage();
        }
