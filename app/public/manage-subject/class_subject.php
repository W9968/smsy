...
$payload = new ClassSubjectController();
$class = new SubjectController();
$group = new GroupeController();

$classes = $class->findMany()->fetchAll();
$groupes = $group->findMany()->fetchAll();

$subjects = $payload->findMany()->fetchAll();
$subjects_count = $payload->findMany()->rowCount();

if (isset($_POST['add'])) {
    $payload->store(new ClassSubject($_POST['time'], $_POST['group'], $_POST['subject']));
    header("Location: .");
}
...
<head>
    ...
    <title>Navsms - class subject </title>
                    ...
                    <form method="POST">
                        <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                            ...
                            <label class="block text-sm">
                                ...
                                <select multiple name="subject" class="overflow-hidden block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-multiselect focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                            ...
                            <label class="block text-sm">
                                ...
                                <select multiple name="group" class="overflow-hidden block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-multiselect focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                                    <?php foreach ($groupes as $key) { ?>
                                        <option value="<?php echo $key['_uid'] ?>"><?php echo $key['class_name'] . $key['class_level'] . " " . $key['dep_id'] ?></option>
                                    <?php } ?>
                                </select>
                            </label>
                            <!-- Hour -->
                            ...
                            <label class="block text-sm">
                                ...
                                <select name="time" class="overflow-hidden block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-multiselect focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                                    <?php foreach (array("8:30", "10", "11:30", "13", "14:30", "16", "17:30") as $key) { ?>
                                ...
                                <table class="w-full whitespace-no-wrap">
                                    <thead>
                                        <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
                                            ...
                                            <th class="px-4 py-3">Begins At</th>
                                        ...
                                        ?>
                                            <tr class="text-gray-700 dark:text-gray-400">
                                                <td class="px-4 py-3 text-sm">
                                                    <?php echo $group->findOne($subject['class_id'])['class_name'] . $group->findOne($subject['class_id'])['class_level'] . " " . $group->findOne($subject['class_id'])['dep_id'] ?>
                                                </td>
                                                <td class="px-4 py-3 text-sm">
                                                    <?php echo $subject['subject_id'] ?>
                                                </td>
                                                <td class="px-4 py-3 text-sm">
                                                    <?php echo $subject['begin_period'] ?>
                                                </td>
                                                <td class="px-4 py-3">
                                        ...
                                        <div x-show="isModalOpen" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 transform translate-y-1/2" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0  transform translate-y-1/2" @click.away="closeModal" @keydown.escape="closeModal" class="w-full px-6 py-4 overflow-hidden bg-white rounded-t-lg dark:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl" role="dialog" id="modal">
                                            ...
                                            <div class="mt-4 mb-6">
                                                ...
                                                <p class="text-sm text-gray-700 dark:text-gray-400">
                                                    Are you sure you want to delete <b>[<?php echo $group->findOne($subject['class_id'])['class_name'] . $group->findOne($subject['class_id'])['class_level'] . " " . $group->findOne($subject['class_id'])['dep_id'] ?>]</b> <?php echo $subject['subject_id'] ?> session that starts at <b><?php echo $subject['begin_period'] ?>AM</b>, if yes please confirm your option.
                                                </p>
                                            </div>
