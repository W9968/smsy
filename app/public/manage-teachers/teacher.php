...
$teacher_count = $payload->findMany()->rowCount();

$assigned = $payload->getAssignedTeacher()->fetchAll();

if (isset($_POST['assign'])) {
	$payload->assign(new Teacher($_POST['teacher'], $_POST['class'], $_POST['salary']));
}
...
<head>
	...
	<title>Navasms - manage techers</title>
	...
	<div class="flex h-screen bg-gray-50 dark:bg-gray-900" :class="{ 'overflow-hidden': isSideMenuOpen}">
		...
		<aside class="fixed inset-y-0 z-20 flex-shrink-0 w-64 mt-16 overflow-y-auto bg-white dark:bg-gray-800 md:hidden" x-show="isSideMenuOpen" x-transition:enter="transition ease-in-out duration-150" x-transition:enter-start="opacity-0 transform -translate-x-20" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in-out duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0 transform -translate-x-20" @click.away="closeSideMenu" @keydown.escape="closeSideMenu">
			<div class="py-4 text-gray-500 dark:text-gray-400">
				...
				<ul>
					<li class="relative px-6 py-3">
						<a class="inline-flex items-center w-full text-sm font-semibold text-gray-800 transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200 dark:text-gray-100" href="../manage-subject/index.php">
							<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
								<path stroke-linecap="round" stroke-linejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
							</svg>
							<span class="ml-4">Manage Subjects</span>
				...
				<ul>
					<li class="relative px-6 py-3">
						<span class="absolute inset-y-0 left-0 w-1 bg-purple-600 rounded-tr-lg rounded-br-lg" aria-hidden="true"></span>
						<a class="inline-flex items-center w-full text-sm font-semibold text-gray-800 transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200 dark:text-gray-100" href="../manage-student/index.php">
							<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
								<path stroke-linecap="round" stroke-linejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
							</svg>
							<span class="ml-4">Manage Students</span>
			...
			<main class="h-full pb-16 overflow-y-auto">
				<div class="container grid px-6 mx-auto">
					...
					<form method="POST">
						...
						if ($teacher_count != 0) {
						?>
							<div class="w-full overflow-hidden rounded-lg <?php if ($teacher_count == 0)  echo "";
																			else echo "shadow-xs"; ?> ">
													...
													<td class="px-4 py-3">
														<div class="flex items-center text-sm">
															...
															<div class="relative hidden w-8 h-8 mr-3 rounded-full md:block">
																<img class="object-cover w-full h-full rounded-full" src="../../uploads/<?php echo $teacher['profile_picture'] ?>" alt="profile picture" loading="lazy" />
																<div class="absolute inset-0 rounded-full shadow-inner" aria-hidden="true"></div>
															</div>
															<div>
																<p class="font-semibold"><?php echo  $teacher['first_name'] . " " . $teacher['last_name'] ?></p>
																<p class="text-xs text-gray-600 dark:text-gray-400">
																	<?php echo $teacher['cin'] ?>
																</p>
															</div>
														</div>
													</td>
						...
						if ($teacher_count != 0) {
						?>
							<div class="w-full overflow-hidden mt-8 rounded-lg <?php if ($teacher_count == 0)  echo "";
																				else echo "shadow-xs"; ?> ">
								<div class="w-full overflow-x-auto">
												...
												<tr class="text-gray-700 dark:text-gray-400">
													<td class="px-4 py-3">
														<div class="flex items-center text-sm">
															...
															<div>
																<input name="teacher" value="<?php echo $teacher['_uid'] ?>" hidden />
																<p class="font-semibold"><?php echo $teacher['first_name'] . " " . $teacher['last_name'] ?></p>
																<p class="text-xs text-gray-600 dark:text-gray-400">
																	<?php echo $teacher['cin'] ?>
																</p>
														...
														<div class="flex items-center text-sm">
															...
															<label class="block text-sm">
																<div class="text-gray-500 focus-within:text-purple-600 dark:focus-within:text-purple-400">
																	<input name="salary" type="text" placeholder="salary..." class="block w-full pl-2 text-sm text-black dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray form-input" />
																</div>
															</label>
														</div>
													</td>
